...
const debounceThreshold = 300; // Adjust the debounce threshold in milliseconds
...
function handleButtons(buttons) {
  const currentTime = performance.now(); // Use performance.now() for high-resolution timestamps

  // Check if it's been at least debounceThreshold milliseconds since the last button click
  if (currentTime - lastButtonClickTime > debounceThreshold) {
...
function handleArrows(axes) {
  const axisThreshold = 0.5;

  // Map axes to arrow directions
  const axesMapping = {
    // Vertical axis
    1: { up: 'up', down: 'down', threshold: axisThreshold },
    // Horizontal axis
    0: { left: 'left', right: 'right', threshold: axisThreshold }
  };

  // Check each axis for arrow key presses
  Object.entries(axesMapping).forEach(([axisIndex, { up, down, left, right, threshold }]) => {
    const axisValue = axes[axisIndex];
    if (axisValue < -threshold) {
      // Arrow key is pressed in the up or left direction
      if (up) {
        snake.setDir(0, -1);
      }
      if (left) {
      ...
      // Arrow key is pressed in the down or right direction
      if (down) {
        snake.setDir(0, 1);
      }
      if (right) {
...
function gameLoop() {
  if (controllerIndex !== null) {
    ...
    handleArrows(gamepad.axes);
  }

  // Threshold for considering an axis as pressed
